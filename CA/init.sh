...
# ROOT CA CREATION
#
#openssl req -config $root_ca_cnf -new -$digest -key $__kkey -out $root_ca_csr
...
# openssl req -new -nodes -utf8 -sha512 -days 36500 -batch -x509 -config x509.genkey -outform DER -out signing_key.x509 -keyout 

# cp $root_ca_pk "$__kdir/build/certs/signing_key.x509"
# cp $root_ca_der "$__kdir/build/certs/signing_key.priv"
...
kern_ca_cert="$dir/signing_key.crt"
kern_ca_csr="$dir/signing_key.csr"
